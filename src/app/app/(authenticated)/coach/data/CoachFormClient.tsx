...
export default function CoachFormClient({ mode, username }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError(null);
    try {
      if (isEditing) await updateCoach(form.username, payload, photoFile);
      else await addCoach(payload, form.password, photoFile);
      router.push("/app/coach/data");
    } catch (e) {
      setSubmitError(e instanceof Error ? e.message : "Failed to save coach");
      setSubmitting(false);
    }
  };
      ...
      <div className="space-y-6 max-w-5xl">
        ...
        <FormSection title="Akun & Foto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              {isEditing && editing?.photo && !photoFile && (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={fileUrl(editing.photo)}
        ...
        <div className="flex items-center justify-end gap-2 pt-2 pb-4">
          ...
          <Button variant="primary" onClick={handleSubmit} disabled={submitting}>
            {submitting ? "Saving…" : isEditing ? "Save Changes" : "Add Coach"}
          </Button>
        </div>
      </div>
    </>
